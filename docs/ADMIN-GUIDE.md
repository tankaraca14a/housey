...
*Last updated: the core admin-flow screenshots (01–20) are regenerated by `node scripts/generate-admin-guide-screenshots.mjs` against a local copy of the housey admin page. The newer review/site/inbox screenshots (24, 30–37) are captured during the feature commits that ship them and live alongside the code. If anything ever looks different from what's pictured here, the page itself is the source of truth.*
