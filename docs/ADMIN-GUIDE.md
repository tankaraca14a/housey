...
> **Don't want to translate yourself?** Send the raw review to Mihaela — see §6.9 below. She translates on her Mac and publishes it in all five languages for you.
...
*Last updated: every screenshot in this document is regenerated by two scripts run against a local copy of the housey admin page. `node scripts/generate-admin-guide-screenshots.mjs` produces shots 01–23 (the admin login + bookings + photos + first-pass review form); `node scripts/generate-feature-screenshots.mjs` produces shots 24, 25, 30–37 (RTL review, multi-language public-site walkthrough, review lang badge + translations, public-side toggle, /submit-review form, translation inbox, publish-with-translations panel). If anything ever looks different from what's pictured here, the page itself is the source of truth.*
